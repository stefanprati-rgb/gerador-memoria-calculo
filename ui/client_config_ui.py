...
def render_client_config(orchestrator):
    ...
    # 2.5 Operações em Lote
    st.markdown("---")
    st.markdown("### 📤 Operações em Lote (Excel/CSV)")
    st.caption("Agilize o preenchimento baixando as UCs atuais e enviando o arquivo preenchido.")
    
    col_dl, col_up = st.columns(2)
    
    with col_dl:
        # Template com as UCs atuais
        # Usamos sep=';' e encoding 'utf-8-sig' para compatibilidade total com Excel PT-BR
        csv_data = current_mapping.to_csv(index=False, sep=";").encode("utf-8-sig")
        st.download_button(
            "⬇️ Baixar Planilha p/ Preencher",
            data=csv_data,
    ...
    with col_up:
        uploaded_file = st.file_uploader("Subir dados preenchidos", type=["csv", "xlsx"], label_visibility="collapsed")
        if uploaded_file:
            try:
                # Carregar o arquivo enviado
                if uploaded_file.name.endswith(".csv"):
                    # Tenta detectar separador comum
                    up_df = pd.read_csv(uploaded_file, sep=";", encoding="utf-8-sig")
                    if ENRICHMENT_KEY not in up_df.columns:
                         # Tenta vírgula se ponto e vírgula falhar
                         up_df = pd.read_csv(uploaded_file, sep=",", encoding="utf-8-sig")
                else:
                    up_df = pd.read_excel(uploaded_file)
                
                # Merge inteligente
                if ENRICHMENT_KEY in up_df.columns:
                    # NORMALIZAÇÃO AVANÇADA (Blindagem contra Excel)
                    def _normalize_uc_id(val):
                        if pd.isna(val): return ""
                        s = str(val).strip()
                        # 1. Remove .0 (flutuações de tipo do Excel)
                        s = s.replace(".0", "")
                        # 2. Se for puramente numérico, remove zeros à esquerda para o MATCH
                        # (Isso permite que '000123' dê match com '123' vindo do Excel)
                        if s.isdigit():
                            return s.lstrip('0')
                        return s

                    # Criamos colunas temporárias de match normalizado
                    up_df["_match_key"] = up_df[ENRICHMENT_KEY].apply(_normalize_uc_id)
                    current_mapping["_match_key"] = current_mapping[ENRICHMENT_KEY].apply(_normalize_uc_id)
                    
                    # Logica: O que está no arquivo SOBRESCREVE o que está na tela (MERGE por match_key)
                    # 1. Identificar quais UCs da tela estão sendo "atualizadas" pelo upload
                    ids_to_update = up_df["_match_key"].unique()
                    
                    # 2. Preservar o que NÃO está no upload (as 45 UCs do exemplo do usuário ficam aqui)
                    mapping_remanescente = current_mapping[~current_mapping["_match_key"].isin(ids_to_update)]
                    
                    # 3. Remover a coluna de match temporária antes de unir (para manter o DF limpo)
                    up_df = up_df.drop(columns=["_match_key"])
                    mapping_remanescente = mapping_remanescente.drop(columns=["_match_key"])
                    
                    # 4. Juntar (CONCAT preserva colunas novas, ex: 'Responsável')
                    final_df = pd.concat([mapping_remanescente, up_df], ignore_index=True)
                    
                    st.session_state.mapping_df = final_df
                    st.success(f"✅ {len(up_df)} registros mesclados. Novas colunas e dados preservados!")
                    st.rerun()
                else:
                    st.error(f"O arquivo enviado não contém a coluna identificadora '{ENRICHMENT_KEY}'.")
